...
// Todo es paralelo a la hoja y tiene volumen real (ancho, alto y profundidad),
// con luz propia: logotipo extruido, bloques del carrusel, sólidos de sección y
// botones como monedas. Una hoja oscura con rejilla dorada en cascada tapa el
// CV impreso, y la luz de la escena se adapta a la luz real que ve la cámara.
...
// Medidas (unidades de la hoja) y alturas sobre el papel (z).
const COVER_MARGIN = 1.04; // la hoja sobresale un poco para tapar bien el CV
const GRID = { cells: 12, opacity: 0.32, secondsPerCell: 3 }; // cascada como en «Mi enfoque»
const LOGO = { w: 0.94, depth: 0.045, z: 0.07, fy: 0.72 };
// Carrusel apretado: las tarjetas se solapan y la central queda encima.
const CARD = { w: 0.44, h: 0.31, depth: 0.035, fy: 0.08, spacing: 0.25, spacingFar: 0.19, zCenter: 0.18, zStep: 0.07 };
const PLAQUE = { w: 0.5, h: 0.083, fy: -0.27 };
const SOLID = { size: 0.085, z: 0.11, fy: -0.5, gap: 0.31, labelFy: -0.64 };
const COIN = { r: 0.056, depth: 0.032, z: 0.075, spread: 0.64, fyEnds: -0.72, fyDip: 0.3 };
...
export const registerWorld = (AFRAME) => {
  ...
  AFRAME.registerComponent('rh-world', {
    ...
    // Etiqueta plana apoyada en la hoja.
    flatLabel(canvas, w, h) {
    ...
    build() {
      ...
      this.shadowTex = this.texture(drawShadow());

      // Luz propia de la escena, fija respecto a la hoja: da volumen a los sólidos.
      const ambient = new THREE.AmbientLight(0xffffff, 0.55);
      const sun = new THREE.DirectionalLight(0xffffff, 1.25);
      sun.position.set(-0.5, 0.9, 1.4);
      const sunTarget = new THREE.Object3D();
      sun.target = sunTarget;
      const rim = new THREE.DirectionalLight(0xfff1d6, 0.45);
      rim.position.set(0.8, -0.6, 0.6);
      rim.target = sunTarget;
      root.add(ambient, sun, rim, sunTarget);
      this.lights = { ambient, sun, rim };
      // Estado suavizado de la luz real (valores iniciales neutros).
      ...
      // Hoja en blanco (tapa el CV). Pivote en el borde superior para el barrido.
      this.coverPivot = new THREE.Group();
      this.coverMat = new THREE.MeshBasicMaterial({ transparent: true, opacity: 0 });
      this.cover = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), this.coverMat);
      this.cover.renderOrder = 0;
      this.coverPivot.add(this.cover);
      this.gridTex = this.texture(drawGridCell());
      this.gridTex.wrapS = THREE.RepeatWrapping;
      this.gridTex.wrapT = THREE.RepeatWrapping;
      this.gridMat = new THREE.MeshBasicMaterial({
        map: this.gridTex,
        alphaMap: this.texture(drawGridFade()),
        transparent: true,
        depthWrite: false,
        opacity: 0,
      });
      ...
      this.coverPivot.add(this.grid);
      this.wipe = new THREE.Mesh(
        new THREE.PlaneGeometry(1.1, 0.012),
        new THREE.MeshBasicMaterial({ color: color(PALETTE.gold), transparent: true, depthWrite: false, opacity: 0 })
      );
      this.wipe.renderOrder = 3;
      root.add(this.coverPivot, this.wipe);
      ...
      this.cards = CAROUSEL_ITEMS.map((item, index) => {
        ...
        const cap = lit('#0e0e0e', { roughness: 0.7 });
        const side = lit(accentColor(item.accent), { roughness: 0.35, metalness: 0.25 });
        const body = new THREE.Mesh(
          new THREE.ExtrudeGeometry(roundedShape(CARD.w, CARD.h, 0.02), { depth: CARD.depth, bevelEnabled: false, curveSegments: 8 }),
          [cap, side]
        );
        ...
        return { item, group, shadow: this.shadowPlane() };
      });

      // Rótulo del carrusel, apoyado en la hoja.
      ...
      // Sólidos de sección que giran sobre la hoja.
      this.solids = WORLD_TILES.map((tile) => {
        ...
        const mat = lit(accentColor(tile.color), { roughness: 0.6, metalness: 0.05, flatShading: true });
        const mesh = new THREE.Mesh(geometry, mat);
        const group = this.track(new THREE.Group(), [mat]);
        group.add(mesh);
        root.add(group);
        const label = this.flatLabel(drawSolidLabel(tile), 0.26, 0.056);
        this.addHit([mesh, label], { kind: 'panel', id: tile.id, group });
        return { tile, group, mesh, label, shadow: this.shadowPlane() };
      });

      // Botones como monedas gruesas (cilindros) con icono en la cara superior.
      this.coins = LINK_BUTTONS.map((button, index) => {
        const side = lit(accentColor(button.color), { roughness: 0.3, metalness: 0.35 });
        const caps = lit('#0e0e0e', { roughness: 0.6 });
        ...
        const label = new THREE.Mesh(new THREE.PlaneGeometry(COIN.r * 2.4, COIN.r * 0.6), labelMat);
        ...
        root.add(group);
        this.addHit([mesh, face], { kind: 'link', button, group });
        return { button, group, label, shadow: this.shadowPlane(), phase: index * 0.9 };
      });
    ...
    buildLogo() {
      ...
      const face = lit(PALETTE.cream, { roughness: 0.45, metalness: 0.05 });
      const side = lit(PALETTE.gold, { roughness: 0.28, metalness: 0.55 });
      const mesh = new THREE.Mesh(geometry, [face, side]);
      this.logo = this.track(new THREE.Group(), [face, side]);
      this.logo.add(mesh);
      this.root.add(this.logo);
      this.logoShadow = this.shadowPlane();
    ...
    layout() {
      ...
      const coverW = COVER_MARGIN;
      const coverH = this.aspect * COVER_MARGIN;
      this.coverH = coverH;
      this.cover.scale.set(coverW, coverH, 1);
      this.cover.position.y = -coverH / 2;
      this.grid.scale.set(coverW, coverH, 1);
      this.grid.position.set(0, -coverH / 2, 0.0005);
      this.gridTex.repeat.set(GRID.cells, GRID.cells * (coverH / coverW));
      this.coverPivot.position.y = coverH / 2;
      this.coverMat.map?.dispose();
      this.coverMat.map = this.texture(drawCover(this.aspect));
      this.coverMat.needsUpdate = true;

      this.logoY = LOGO.fy * halfH;
      this.carouselY = CARD.fy * halfH;
      this.plaque.position.set(0, PLAQUE.fy * halfH, 0.004);
      this.solids.forEach((s, i) => {
        s.base = new THREE.Vector3((i - 1) * SOLID.gap, SOLID.fy * halfH, SOLID.z);
        s.label.position.set(s.base.x, SOLID.labelFy * halfH, 0.004);
      });
    ...
    setOpacity(group, value) {
      group.userData.materials.forEach((m) => { m.opacity = value; });
    ...
    applyAmbient(dt) {
      ...
      ambient.intensity = 0.15 + 0.5 * level;
      sun.intensity = 0.3 + 1.0 * level;
      rim.intensity = 0.1 + 0.3 * level;
    ...
    tick(time, dt = 16) {
      ...
      // 1) Barrido de la hoja en blanco.
      const cw = at(0, 560);
      const ce = easeOutCubic(cw);
      this.coverPivot.scale.y = Math.max(0.0001, ce);
      this.coverMat.opacity = fade * Math.min(1, cw * 4);
      this.wipe.position.set(0, this.coverH / 2 - this.coverH * ce, 0.004);
      this.wipe.material.opacity = !instant && cw > 0 && cw < 1 ? fade : 0;
      this.gridMat.opacity = GRID.opacity * fade * clamp01((cw - 0.3) * 2);
      if (!still) this.gridTex.offset.y = (sec / GRID.secondsPerCell) % 1;

      // Luz de la escena según la luz real de la habitación.
      this.sampleAmbient(now);
      this.applyAmbient(dt);

      // 2) El logotipo emerge de la hoja.
      const lp = at(380, 900);
      const le = instant ? lp : easeOutBack(lp);
      const lz = LOGO.z * le + (still || lp < 1 ? 0 : 0.008 * Math.sin(sec * 1.2));
      this.logo.position.set(0, this.logoY, lz);
      this.logo.rotation.set(0, 0, 0);
      this.logo.scale.setScalar((0.7 + 0.3 * le) * this.pressScale(this.logo));
      const la = fade * clamp01(lp * 3);
      this.setOpacity(this.logo, la);
      this.logoShadow.position.set(0, this.logoY - 0.015, 0.002);
      this.logoShadow.scale.set(LOGO.w * 1.05, this.logoH * 1.6, 1);
      this.logoShadow.material.opacity = la * 0.5 * clamp01(lz / LOGO.z);

      // 3) Carrusel.
      this.updateCarousel(dt, at(750, 750), fade, now, instant);

      // 4) Sólidos de sección.
      this.solids.forEach((s, i) => {
        const p = at(1150 + i * 110, 700);
        const e = instant ? p : easeOutBack(p);
        const g = s.group;
        const z = s.base.z * e + (still || p < 1 ? 0 : 0.01 * Math.sin(sec * 1.5 + i));
        g.position.set(s.base.x, s.base.y, z);
        g.scale.setScalar(Math.max(0.001, e) * this.pressScale(g));
        if (!still) {
        ...
        s.shadow.position.set(s.base.x, s.base.y - 0.01, 0.002);
        s.shadow.scale.set(0.14, 0.09, 1);
        s.shadow.material.opacity = alpha * 0.5 * clamp01(z / SOLID.z);
      });

      // 5) Monedas: salen desde el centro y flotan paralelas a la hoja.
      this.coins.forEach((c, i) => {
        const p = at(1350 + i * 75, 800);
        const move = easeOutCubic(p);
        const pop = instant ? p : easeOutBack(p);
        const bob = still || p < 1 ? 0 : 1;
        const g = c.group;
        const z = c.base.z * pop + bob * 0.012 * Math.sin(sec * 1.7 + c.phase);
        g.position.set(c.base.x * move, this.carouselY + (c.base.y - this.carouselY) * move, z);
        g.scale.setScalar((0.35 + 0.65 * pop) * this.pressScale(g));
        const alpha = fade * clamp01(p * 3);
        this.setOpacity(g, alpha);
        c.shadow.position.set(g.position.x, g.position.y - 0.012, 0.002);
        c.shadow.scale.set(0.17, 0.13, 1);
        c.shadow.material.opacity = alpha * 0.5 * clamp01(z / COIN.z);
      });
    ...
    updateCarousel(dt, appear, fade, now, instant) {
      ...
      const ae = instant ? appear : easeOutBack(appear);
      this.cards.forEach((card, i) => {
        const rel = mod(i - c.offset + n / 2, n) - n / 2;
        const a = Math.abs(rel);
        const g = card.group;
        const hover = !this.reduced && appear >= 1 ? Math.max(0, 1 - a) * 0.012 * Math.sin(now / 520) : 0;
        // Cada paso hacia fuera baja un escalón: la central queda encima y pisa a las demás.
        const z = Math.max(0.012, CARD.zCenter - Math.min(a, 3) * CARD.zStep * (a > 1 ? 0.75 : 1)) * ae + hover;
        const x = Math.sign(rel) * (a <= 1 ? a * CARD.spacing : CARD.spacing + (a - 1) * CARD.spacingFar);
        g.position.set(x, this.carouselY, z);
        g.rotation.set(0, 0, 0);
        g.scale.setScalar((1 - Math.min(a, 2.5) * 0.12) * (0.55 + 0.45 * ae) * this.pressScale(g));
        const alpha = (a < 2.4 ? 1 : Math.max(0, 1 - (a - 2.4) / 0.5)) * clamp01(appear * 2.5) * fade;
        this.setOpacity(g, alpha);
        card.shadow.position.set(g.position.x, this.carouselY - 0.02, 0.002);
        card.shadow.scale.set(0.58 * g.scale.x, 0.42 * g.scale.x, 1);
        card.shadow.material.opacity = alpha * 0.6 * clamp01(z / CARD.zCenter);
      });
